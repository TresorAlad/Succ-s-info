          ...
          if (req.url === '/api/contact' && req.method === 'POST') {
            ...
            req.on('end', async () => {
              try {
                ...
                const { type, name, email, subject, message, formationDetails } = parsedBody;
                ...
                if (!smtpEmail || !smtpPass) {
                  ...
                  res.end(JSON.stringify({ error: "Configuration email manquante." }));
                  return;
                }
                ...
                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.end(JSON.stringify({ message: 'Envoyé avec succès !' }));
              } catch (error) {
                console.error("API Error:", error);
                res.statusCode = 500;
                res.setHeader('Content-Type', 'application/json');
                res.end(JSON.stringify({ error: "Erreur serveur lors de l'envoi." }));
              }
            });
            return;
          }
